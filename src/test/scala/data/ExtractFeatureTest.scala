...
class ExtractFeatureTest extends FunSuite with Matchers with BeforeAndAfterAll {
	...
	override protected def beforeAll(): Unit = {
		...
		feature = new ExtractFeature(data.map(Vectors.dense))
	}
	...
	test("computeAvgAbsDifference") {
		var avgAbsDiff = feature.computeAvgAbsDifference(data)
				  		.map(x => math.round(x*100)/100.0)
	...
	test("computeResultantAcc") {
		var resultant = math.round(feature.computeResultantAcc(data)*100)/100.0
